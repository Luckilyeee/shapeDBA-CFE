...
from nte.data.real.univariate.binary.BirdChicken import BirdChickenDataset
...
from nte.data.real.univariate.multi_class.Plane import PlaneDataset
...
SNS_CMAP = ListedColormap(sns.light_palette('orange').as_hex())
...
def dataset_mapper(DATASET):
    ...
    if DATASET in ['GunPoint', "1"]:
        ...
        dataset = CBFDataset()
    elif DATASET in['Plane', '6']:
        dataset = PlaneDataset()
    elif DATASET in['BirdChicken', '7']:
        dataset = BirdChickenDataset()
    else:
...
def plot_cmap(data, pertuebated_data, saliency):
    ...
    plt.clf()
    fig = plt.gcf()
    im = plt.imshow(saliency.reshape([1, -1]), cmap=SNS_CMAP, aspect="auto", alpha=0.5,
    ...
    plt.plot(data, lw=1, label = "Original", color = 'green')
    plt.plot(pertuebated_data, lw=1, label = 'Counterfactual', color = 'red')
    plt.grid(False)
