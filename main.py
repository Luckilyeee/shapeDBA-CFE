...
import shortuuid
from nte.models.saliency_model.counterfactual_ShapeDBA import CFExplainer_ShapeDBA
import random
...
if __name__ == '__main__':
    ...
    if args.algo == 'cf':
        explainer = CFExplainer_ShapeDBA(background_data=bg_data[:bg_len], background_label=bg_label[:bg_len],
    ...
    res_path = f"shapedba/{args.dataset}/"
    ...
    for ind, (original_signal, original_label) in ds: #original_signal is from the test set
        try:
            ...
            # ori_perturbated_res = ori_perturbated_res.cpu().detach().numpy().flatten()
            # cf_res0.append(ori_perturbated_res)
